...
class SWATS(Optimizer):
    r"""Implements Switching from Adam to SGD_noschedule technique. Proposed in
    `Improving Generalization Performance by Switching from Adam to SGD_noschedule`
    by Nitish Shirish Keskar, Richard Socher (2017).
    The method applies Adam in the first phase of the training, then
    switches to SGD_noschedule when a criteria is met.
    Implementation of Adam and SGD_noschedule update are from `torch.optim.Adam` and
    `torch.optim.SGD_noschedule`.
    """

    def __init__(self, params, lr=1e-3, betas=(0.9, 0.999), eps=1e-8,
                 weight_decay=0, amsgrad=False, verbose=False,
                 nesterov=False, decoupled_wd=False):
    ...
    def step(self, closure=None):
        ...
        for group in self.param_groups:
            for w in group['params']:
                ...
                # if its SGD_noschedule phase, take an SGD_noschedule update and continue
                if group['phase'] == 'SGD_noschedule':
                ...
                if pg != 0:
                    ...
                    # checking criteria of switching to SGD_noschedule training
                    if state['step'] > 1 and \
                            corrected_exp_avg.allclose(scaling, rtol=1e-6) and \
                            corrected_exp_avg > 0:
                        group['phase'] = 'SGD_noschedule'
                        group['lr'] = corrected_exp_avg.item()
                        if group['verbose']:
                            print('Switching to SGD_noschedule after '
                                  '{} steps with lr {:.5f} '
